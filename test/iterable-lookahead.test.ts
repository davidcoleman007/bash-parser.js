import { assert, assertEquals, assertThrows } from '~/utils/assert';
...
describe('iterable-lookahead', async (t) => {
  const expected1 = [{
    item: 1,
    ahead: 2,
    behind: undefined,
  ...
  it('resulting iterable is equivalent to the initial one', async () => {
    const arr = [];
  ...
  it('resulting iterable has a ahead method', async () => {
    const it = iterableLookahead(fromArray([]));
    assertEquals(typeof it.ahead, 'function');
  });

  it('resulting iterable has a behind method', async () => {
    const it = iterableLookahead(fromArray([]));
    assertEquals(typeof it.behind, 'function');
  });

  it('behind and ahead indexes defaults to 1', async () => {
  ...
  it('behind and ahead works with indexes smaller than size', async () => {
  ...
  it('behind and ahead use index', async () => {
    const it = iterableLookahead(fromArray([1, 2, 3, 4]), 2);
    const result = [];
  ...
  it('behind and ahead works with size greater than array', async () => {
  ...
  it('throws if lookahead or lookbehind are over size', async () => {
  ...
  it('throws if size is smaller than 1', async () => {
  ...
  it('throws if lookahead or lookbehind are <= 0', async () => {
    const it = iterableLookahead(fromArray([1, 2, 3, 4]), 1);
    const err1 = assertThrows(() => it.ahead(-2)) as Error;
    const err2 = assertThrows(() => it.behind(-2)) as Error;
